...
public class PointsOfInterest {
	...
	private static List<MapNode> nodes = new ArrayList<MapNode>(); 
...
//		double random = rng.nextDouble();
		...
		for(int i = 0; i < poiLists.size(); i++) {
			if(poiLists.get(i) != null) {
				temp = poiLists.get(i).remove(0);
				poiLists.get(i).add(temp);
...
//		} while (okMapNodeTypes != null && !node.isType(okMapNodeTypes));
	...
	public MapNode getNodeByCoord(Coord c) {
		for(MapNode n : nodes) {
			if(n.getLocation().toString().equals(c.toString())) {
				return n;
			}
		}
